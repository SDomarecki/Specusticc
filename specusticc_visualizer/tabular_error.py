...
class TabularError:
    def count_errors(self, true_data: pd.DataFrame, prediction_data: dict):
        ...
        for model_name, model_preds in prediction_data.items():
            ...
            errors_str = " & ".join("{:.2f}".format(x) for x in mean_errors)
            print(f"{model_name} & {errors_str} \\\\")

    def count_one_model_errors(
        self, true_data: pd.DataFrame, one_model_predictions: pd.DataFrame
    ):
        ...
        for column in columns:
            ...
            errors = np.append(
                errors,
                self.count_one_prediction_errors(true_data, one_prediction),
                axis=0,
            )
    ...
    def count_one_prediction_errors(
        self, true_data: pd.DataFrame, one_prediction: pd.DataFrame
    ) -> np.array:
