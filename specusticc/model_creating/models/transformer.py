...
class ModelTransformer:
    ...
    def build_model(self, dropout_rate=0.01, neurons=20, num_stacks=4):
        ...
        encoder_inputs = Layers.Input(
            shape=(self.context_timesteps, self.context_features)
        )
        encoder = Encoder(num_stacks, d_model, num_heads, dff, rate=dropout_rate)
        encoder_outputs = encoder(encoder_inputs)

        d_model = num_heads = dff = self.input_features
        decoder_inputs = Layers.Input(shape=(self.input_timesteps, self.input_features))
        decoder = Decoder(
