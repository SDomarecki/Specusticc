...
class LSTMEncoderDecoder:
    ...
    def build_model(self, optimizer="adam", middle_layers=1, dropout_rate=0.2):
        # define training encoder
        encoder_inputs = L.Input(
        ...
        decoder_lstm = L.LSTM(
            self.context_features, return_sequences=True, name="Decoder_LSTM"
        )
        decoder_outputs = decoder_lstm(middle_decoder, initial_state=encoder_states)
        decoder_outputs = L.Flatten()(decoder_outputs)
        decoder_dense = L.Dense(
            self.output_timesteps, activation="linear", name="Dense_output"
        )
