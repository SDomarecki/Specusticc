...
class Configer:
    ...
    def _create_loader_config(self):
        ...
        loader_config.context_tickers = self._dict_config["import"]["context"][
            "tickers"
        ]
    ...
    def _create_preprocessor_config(self):
        ...
        preprocessor_config.input_columns = self._dict_config["import"]["input"][
            "columns"
        ]
        if "date" not in preprocessor_config.input_columns:
            preprocessor_config.input_columns.append("date")
        preprocessor_config.output_columns = self._dict_config["import"]["target"][
            "columns"
        ]
        if "date" not in preprocessor_config.output_columns:
            preprocessor_config.output_columns.append("date")
        preprocessor_config.context_columns = self._dict_config["import"]["context"][
            "columns"
        ]
        if "date" not in preprocessor_config.context_columns:
            preprocessor_config.context_columns.append("date")

        preprocessor_config.context_features = (
            len(self._dict_config["import"]["context"]["columns"]) - 1
        )  # minus data

        train_date_range = DateRange(
            self._dict_config["import"]["train_date"]["from"],
        ...
        preprocessor_config.window_length = self._dict_config["preprocessing"][
            "window_length"
        ]
        ...
        preprocessor_config.features = (
            len(self._dict_config["import"]["input"]["columns"]) - 1
        )  # minus data
        self._configs.preprocessor = preprocessor_config

    def _create_agent_config(self):
        agent_config = AgentConfig()
        agent_config.input_timesteps = self._dict_config["preprocessing"][
            "window_length"
        ]
        ...
        agent_config.context_timesteps = self._dict_config["preprocessing"][
            "window_length"
        ]
        ...
        agent_config.hyperparam_optimization_method = self._dict_config["agent"][
            "hyperparam_optimization_method"
        ]
